...
#
# adapted from version in dynaflow directory
#
...
Usage: $0 [OPTIONS] CASE_DIR BASECASE CASE_PREFIX
  Options:
    ...
    -O | --launcherO  Defines the launcher for OLF
...
OPTIONS=cdho:vsH:O:
LONGOPTS=cleanup,debug,help,output:,verbose,sequential,launcherH:,launcherO:
...
c=n d=n h=n outDir="RESULTS" v=n s=n H="hades2.sh" O="itools"
while true; do
    case "$1" in
        -c|--cleanup)
            c=y
...
if [ $v = "y" ]; then
    echo "$0: Called with OPTIONS: cleanup: $c, debug: $d, help: $h, output: $outDir, verbose: $v, sequential: $s, launcherH: $H, launcherO: $O"
...
declare -a OPTS
if [ $c = "y" ]; then
    OPTS=("-c" "-o" "$outDir" "-H" "$H" "-O" "$O" -b "$BASECASE")
else
    OPTS=("-o" "$outDir" "-H" "$H" "-O" "$O" -b "$BASECASE")
fi
run_case=$(dirname "$0")/run_one_case.sh

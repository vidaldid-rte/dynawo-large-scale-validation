...
def main():
    ...
    for filepath in files:
        ...
        delta_mean = delta.groupby("VAR").mean(numeric_only=True)

        delta_p95 = delta.groupby("VAR").quantile(0.95, numeric_only=True)

        res1 = (
            [contg]
            + ["ALL"]
            + list(temp_df_max_list)
            + list(delta_p95["DIFF_ABS"].values)
            + list(delta_mean["DIFF_ABS"].values)
        )
        res = res + [res1]
        ...
        for volt_level in volt_levels:
            ...
            temp_df_mean = temp_df.groupby("VAR").mean(numeric_only=True)
            temp_df_p95 = temp_df.groupby("VAR").quantile(0.95, numeric_only=True)
            ...
            for i in range(len(index_list)):
                if index_list[i] == real_index_list[i]:
                    ...
                    temp_df_p95_dict["DIFF_ABS"].insert(i, None)
                    temp_df_mean_dict["DIFF_ABS"].insert(i, None)

            # temp_df_max_list = list(temp_df_max_dict["DIFF"])
            temp_df_p95_list = list(temp_df_p95_dict["DIFF_ABS"])
            temp_df_mean_list = list(temp_df_mean_dict["DIFF_ABS"])
